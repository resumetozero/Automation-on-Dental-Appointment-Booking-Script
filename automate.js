...
(async () => {
        ...
        try {
            ...
            if (filledValue !== dateValue) {
                ...
                await page.evaluate((s, val) => {
                    ...
                    input.value = val;
                    ['input', 'change', 'blur'].forEach(event => input.dispatchEvent(new Event(event, { bubbles: true })));
                }, selector, dateValue);
                filledValue = await page.evaluate(s => document.querySelector(s)?.value, selector);
            }
    ...
    try {
        ...
        let mobileNumber = prompt("Enter Mobile Number (e.g., +14155550132): ").trim();
        while (!/^\+?\d{10,12}$/.test(mobileNumber)) {
            console.log("Invalid number. Enter 10-12 digits, optionally with '+' (e.g., +14155550132).");
            mobileNumber = prompt("Enter Mobile Number (e.g., +14155550132): ").trim();
        }
        ...
        // Extract and list available appointment types (reasons)
        console.log("Extracting available appointment types...");
        ...
        console.log("Available Appointment Types (Reasons):");
        reasons.forEach((reason, index) => console.log(`${index + 1}: ${reason}`));

        const appointmentType = prompt("Enter appointment type or choose from list above: ").trim();
        ...
        // Extract and list available providers
        console.log("Extracting available providers...");
        ...
        const providers = await page.evaluate(() => {
            return Array.from(document.querySelectorAll('[data-testid^="t-provider-selector-item-"]'))
                .map(opt => opt.textContent.trim())
                .filter(text => text.length > 0);
        });
        ...
        const providerType = prompt("Enter provider (e.g., Any Provider) or choose from list above: ").trim();
        ...
        while (continueLoop) {
            ...
            if (slotChoice >= 0 && slotChoice < allSlots.length) {
                ...
                if (prompt("Confirm this slot? (yes/no): ").toLowerCase().trim() === "yes") {
                    ...
                    await typeField('input#mui-5', mobileNumber, "Phone Number");
                    await typeField('input#mui-6', email, "Email");
